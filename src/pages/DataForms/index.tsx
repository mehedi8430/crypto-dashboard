...
// --- UI Component Mocks (Styled with CSS Variables from index.css) ---
const FormControl = ({ children }: { children: React.ReactNode }) => <div>{children}</div>;
const FormItem = ({ children, className }: { children: React.ReactNode, className?: string }) => <div className={cn("flex flex-col space-y-1.5", className)}>{children}</div>;
const FormLabel = ({ children, ...props }: React.ComponentProps<"label">) => <label {...props} className="text-sm font-medium text-foreground">{children}</label>;
const FormMessage = ({ children }: { children?: React.ReactNode }) => <p className="text-xs text-destructive h-4">{children}</p>;
const Button = ({ children, ...props }: React.ComponentProps<"button">) => <button {...props} className={cn("bg-primary hover:bg-primary/90 text-primary-foreground font-bold py-2 px-4 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed", props.className)}>{children}</button>;

const Input = React.forwardRef<HTMLInputElement, React.ComponentProps<"input">>((props, ref) => (
    <input ref={ref} {...props} className={cn(`bg-input border border-border text-foreground rounded-md px-3 py-2 text-sm w-full`, props.disabled ? 'opacity-50 cursor-not-allowed' : '')} />
));
Input.displayName = "Input";

const Textarea = React.forwardRef<HTMLTextAreaElement, React.ComponentProps<"textarea">>((props, ref) => (
    <textarea ref={ref} {...props} className={cn(`bg-input border border-border text-foreground rounded-md px-3 py-2 text-sm w-full`, props.disabled ? 'opacity-50 cursor-not-allowed' : '')} />
));
Textarea.displayName = "Textarea";

const Checkbox = React.forwardRef<HTMLInputElement, { checked?: boolean, onCheckedChange?: (checked: boolean) => void } & Omit<React.ComponentProps<"input">, 'onChange'>>(({ checked, onCheckedChange, ...props }, ref) => (
    <input
        ref={ref}
        type="checkbox"
        checked={checked}
